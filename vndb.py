...
class DBHelper(object):
    ...
    def get_conn(self):
        if self._conn:
            ...
            return self._conn
        try:
            self._conn = MySQLdb.connect(**self._db_conf)
        except Exception as e:
            raise e
        return self._conn
    ...
    def get_cursor(self, cursorclz=None):
        if not self._conn:
            ...
            self._conn.ping()
        try:
            cursor = self._conn.cursor(cursorclass=cursorclz)
        except Exception as e:
            raise e
        return cursor
    ...
    def get_dicts(self, sql, params=None, convert=True):
        if not self._dict_cursor:
            self._dict_cursor = self.get_cursor(MySQLdb.cursors.DictCursor)
        else:
            self._conn.ping()
        try:
    ...
    def execute_batch(self, sql, params, step_size=2000):
        ...
        try:
            ...
            for i in xrange(0, (length + size - 1) / size):
